...
public final class ToolBeltInventoryIntegrationProvider implements InventoryIntegrationProvider {
    private static final String PROVIDER_ID = "slot:toolbelt";
    ...
    public ProviderResult<InventoryHostSession> openHost(InventoryHostContext context) {
        return ProviderResult.unsupported(
                ...
                "ToolBelt contributes carried player sources only"
        );
    }

    @Override
    public List<PlayerInventoryExtension> playerExtensions(PlayerInventoryContext context) {
        if (context == null || context.playerInventory() == null || !ToolBeltSupport.isAvailable()) {
            return List.of();
        }
        ...
        List<ToolBeltSupport.BeltSnapshot> snapshots = ToolBeltSupport.readPlayerBelts(player);
        if (snapshots.isEmpty()) {
            return List.of();
        }
        return List.of(new Extension(snapshots));
    }

    private static final class Extension implements PlayerInventoryExtension {
        private final List<ToolBeltSupport.BeltSnapshot> initialSnapshots;

        private Extension(List<ToolBeltSupport.BeltSnapshot> initialSnapshots) {
        ...
        public List<InventorySourceDescriptor> additionalSources() {
            ...
            for (ToolBeltSupport.BeltSnapshot snapshot : initialSnapshots) {
                sources.add(InventorySourceDescriptor.builder(snapshot.sourceId())
                        ...
                        .diagnostics("toolbelt/" + snapshot.sourceId())
        ...
        private static MutationResult mutateInsert(InventoryMutationRequest request, boolean simulate) {
            ...
            ToolBeltSupport.BeltSnapshot snapshot = ToolBeltSupport.find(request.player(), request.sourceId());
            if (snapshot == null) {
                return MutationResult.blocked("unsupported_source", stack);
            }
            ItemStack remainder = ToolBeltSupport.insertInto(snapshot, stack, simulate);
            if (remainder == null) {
        ...
        private static MutationResult mutateExtract(InventoryMutationRequest request, boolean simulate) {
            ...
            for (ToolBeltSupport.BeltSnapshot snapshot : ToolBeltSupport.readPlayerBelts(request.player())) {
                for (int slot = 0; slot < snapshot.slotCount(); slot++) {
                    ItemStack preview = ToolBeltSupport.peek(snapshot, slot);
                    if (!ItemIdentityMatcher.matchesMovable(preview, request.identity())) {
                        continue;
                    }
                    int amount = request.requestedAmount(preview.getCount());
                    ItemStack extracted = ToolBeltSupport.extract(snapshot, slot, amount, simulate);
        ...
        private static MutationResult extractFromSlot(InventoryMutationRequest request, boolean simulate) {
            ...
            ItemStack preview = ToolBeltSupport.peek(snapshot, request.slotIndex());
            if (preview.isEmpty()) {
                return MutationResult.blocked("no_matching_stack", ItemStack.EMPTY);
            }
            if (request.identity() != null
            ...
            int amount = request.requestedAmount(preview.getCount());
            ItemStack extracted = ToolBeltSupport.extract(snapshot, request.slotIndex(), amount, simulate);
            if (extracted == null || extracted.isEmpty()) {
                return MutationResult.blocked("no_matching_stack", ItemStack.EMPTY);
            }
            return MutationResult.success(extracted);
