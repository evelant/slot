...
/**
 * Reconciles persisted chest claims with loader-specific in-world storage-id
 * markers. Unloaded anchors remain unknown so claims are not pruned merely
 * because a chunk has not been visited yet.
 */
public final class ChestClaimPersistenceReconciliation {
    private ChestClaimPersistenceReconciliation() {
    }

    public static void reconcile(
            MinecraftServer server,
            WorkflowDomainRuntime runtime,
            StorageIdLookup storageIdLookup
    ) {
        if (server == null || runtime == null || storageIdLookup == null) {
            return;
        }
        ...
        for (ClaimedChest chest : claims) {
            ...
            ReconcileResult result = reconcileChest(server, chest, storageIdLookup);
            if (result.deleted()) {
    ...
    private static ReconcileResult reconcileChest(
            MinecraftServer server,
            ClaimedChest chest,
            StorageIdLookup storageIdLookup
    ) {
        ...
        for (ChestAnchor anchor : chest.anchors()) {
            AnchorState state = evaluateAnchor(server, chest.storageId(), anchor, storageIdLookup);
            switch (state) {
        ...
        if (surviving.isEmpty() && !anyUnknown) {
            return new ReconcileResult(true, broken, new LinkedHashSet<>());
        }
        if (broken == 0) {
            return new ReconcileResult(false, 0, new LinkedHashSet<>());
        }
        return new ReconcileResult(false, broken, surviving);
    }

    private static AnchorState evaluateAnchor(
            MinecraftServer server,
            UUID expectedStorageId,
            ChestAnchor anchor,
            StorageIdLookup storageIdLookup
    ) {
        if (anchor == null) {
        ...
        Optional<UUID> actual = storageIdLookup.read(level, pos);
        if (actual.isEmpty()) {
            return AnchorState.BROKEN;
        }
    ...
    public interface StorageIdLookup {
        Optional<UUID> read(ServerLevel level, BlockPos pos);
    }
    ...
    private record ReconcileResult(
            boolean deleted,
            int removedAnchors,
            LinkedHashSet<ChestAnchor> remainingAnchors
    ) {
    }
}
