...
For current core semantics, see [overview.md](overview.md).
...
This layer is now partially landed. It translates session output into a
UI-neutral workspace model. The current implementation is sufficient for
headless composition and the first player workspace slice, while richer
...
This concept is landed for the current NeoForge path. It is still expected to
grow as dual-pane/container and compat-heavy hosts require richer observation
facts.
...
The first production shell path is the LDLib2 player-inventory workspace. LDLib2
owns menu-scoped UI transport and widget composition for this slice; SLOT still
owns authority, request construction, planning, execution, workflow, and
diagnostics.
...
7. A NeoForge screen shell or LDLib workspace renders that model or a compact
   server-owned view model derived from the same authority/session state.
...
10. Routed actions become concrete `InventoryActionRequest` payloads to the
    server when authority mutation is required. In the LDLib workspace, the
    client sends only a narrow RPC command and the server constructs the
    concrete request from live authority.
