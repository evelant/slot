...
import dev.imagio.slot.neoforge.network.SlotSetWantedCountPayload;
...
/**
 * Global GUI shortcut for setting the item under the cursor to wanted=1.
 * EMI gets first shot at hover resolution so recipe widgets and standard
 * slots both work; vanilla container slot hit-testing is the fallback.
 */
public final class SlotHoveredWantedHotkey {
    private static boolean registered;
    private static boolean pressed;

    private SlotHoveredWantedHotkey() {
    }
    ...
    public static void init() {
        ...
        NeoForge.EVENT_BUS.addListener(SlotHoveredWantedHotkey::onKeyPressed);
        NeoForge.EVENT_BUS.addListener(SlotHoveredWantedHotkey::onClientTick);
        registered = true;
    }

    private static void onClientTick(ClientTickEvent.Post event) {
        if (!SlotAtlasKeyMappings.setWantedHoverDown()) {
            pressed = false;
        }
    }

    private static void onKeyPressed(ScreenEvent.KeyPressed.Pre event) {
        if (!SlotAtlasKeyMappings.matchesSetWantedHover(event.getKeyCode(), event.getScanCode())) {
            return;
        }
        ...
        PacketDistributor.sendToServer(new SlotSetWantedCountPayload(
                ref.itemId(),
                ref.comparisonMode(),
    ...
    private static ItemStack hoveredStack(Screen screen) {
        ...
        Slot slot = hoveredVanillaSlot(containerScreen, mouseX, mouseY);
